...
echo '</tr>';
foreach(array('1'=>'Yes','2'=>'No','3'=>'Ask','5'=>'N/A','4'=>'Don\'t Care') as $k=>$v){
...
echo '<label for="doc" class="desc">License Document</label> <select name="doc_alias" id="doc">';
echo '<option value="">--</option>';
foreach($docs as $doc){
	if($_POST['doc_alias']===$doc['alias']){
		$s=' selected="selected"';
	}else{
		$s='';
	}
	echo '<option value="'.$doc['alias'].'"'.$s.'>'.$doc['filename'].'</option>';
}
echo '</select>';
?>
...
<th class="heading">Class Handouts</th>
...
<th class="heading">License Document</th>
</tr>
<?php
		foreach($res as $row){
			$id=$row['id'];
